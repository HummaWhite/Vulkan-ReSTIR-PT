...
public:
	...
	void setFilmSize(glm::ivec2 size) { mFilmSize = size; }
	void setPlanes(float nearZ, float farZ) { mNear = nearZ, mFar = farZ; }
	...
	glm::mat4 viewMatrix(glm::vec3 focus) const { return glm::lookAt(mPos, focus, mUp); }
	glm::mat4 projMatrix() const { return glm::perspective(glm::radians(mFOV), mAspect, mNear, mFar); }
